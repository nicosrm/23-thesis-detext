...
extension UIImage {
    ...
    func toCVPixelBuffer() -> CVPixelBuffer? {
        ...
        guard let graphic = self.cgImage else {
            return nil
        }
...
func overlayBlackBg(image: UIImage) -> UIImage {
    ...
    let drawPosition = CGPoint(
        x: newFrameWidth/2 - image.size.width/2,
        y: newFrameHeight/2 - image.size.height/2
    )
    ...
    return renderer.image { _ in
        ...
        image.draw(in: CGRect(origin: drawPosition, size: image.size))
    }
}
